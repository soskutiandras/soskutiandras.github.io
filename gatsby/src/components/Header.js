...
const HeaderWrapper = styled.div`
  ...
  ${Button} {
    color: ${props => props.scrolled && `${Colors.blue}`};
    border: 1px solid ${props => props.scrolled && `${Colors.blue}`};
    &:hover {
      color: ${props => props.scrolled && `${lighten(0.3, Colors.white)}`};
      border: 1px solid ${props => props.scrolled && `${lighten(0.3, Colors.blue)}`};
    }
    ${media.menuMax`
    `}
    ${media.desktop`
      ...
      &:hover {
        ...
        border: 1px solid ${props => props.scrolled && `${darken(0.3, Colors.blue)}`};
      }
    `}
  }
  ${HireMe} {
...
const Burger = styled.div`
  ...
  color: ${Colors.darker};
  z-index: 2;
  color: ${props => (props.scrolled || props.theme === 'white' ? `${Colors.darker}` : `${Colors.darkest}`)};
  &:hover {
    ${props =>
      props.scrolled || props.theme === 'white' ? lighten(0.3, Colors.darkest) : darken(0.1, Colors.darkest)};
    cursor: pointer;
    opacity: 0.9;
  }
  ${mediaMin.desktop`
  ...
  render() {
    ...
    return (
      <HeaderWrapper theme={theme} scrolled={scrolled}>
        ...
        <ContentWrapper>
          <Logo href="/">
            <img src={Avatar} alt="Soskuti Andras" />
          </Logo>
          <HeaderNav>
